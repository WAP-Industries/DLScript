    ...
    protected internal static Dictionary<string, Keyword> Functions = new()
    {
        ...
        },
        {
            "set", new Keyword(
                ...
                (parameters) => Convert.ToBoolean(ModifyArray(parameters[0], parameters[1], parameters[2]))
             )
        }
    };

    private static object ModifyArray(object _name, object _index, object _value) {
        ...
        // check index
        object index = Lexer.EvalExpr(Convert.ToString(_index), Array.Empty<string>(), false, "number", false);
        if (index == null) 
        ...
        object value = Lexer.EvalExpr(arrtype=="string" ? $"~[{Convert.ToString(_value)}]~":Convert.ToString(_value), 
