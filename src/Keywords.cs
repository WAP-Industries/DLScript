...
class Keywords : DickLang.Compiler {
    ...
    protected internal static Dictionary<string, Keyword> Functions = new()
    {
        {
            "print", new Keyword(
                new TokenInfo(new string[]{"function"}, new string[]{"string"}),
                (parameters)=> {
        ...
        {
            "abort", new Keyword(
                new TokenInfo(new string[]{"function"}),
                (parameters) => {
        ...
        {
            "delete", new Keyword(
                new TokenInfo(new string[]{"function", "args"}, new string[]{"rawstring"}),
                (parameters) => {
        ...
        {
            "if", new Keyword(
                new TokenInfo(new string[]{"function"}, new string[]{"bool", "number"}),
                (parameters)=>{
        ...
        {
            "call", new Keyword(
                new TokenInfo(new string[]{"function", "funcargs"}, new string[]{"rawstring", "*"}),
                (parameters) => {
    ...
    private static object CheckCallParams(Dictionary<string, object> StoredArgs, string FuncArgs, string[] Tokens) {
        ...
        foreach (var arg in RawArgs) {
            ...
            var val = Lexer.EvalExpr(arg, Tokens, Convert.ToString(argtype) == "string", Convert.ToString(argtype));
    ...
    private static bool InLoop(string name) {
        if (LoopInfo.Key == -1) {
            Error.RunTimeError("Syntax", $"Illegal {name} statement");
            return false;
        }
        return true;
    }
